...
### Changed
- Refactored `LeaderboardService` to delegate sorting logic to the repository layer.
- Replaced in-memory sorting with `findAllByOrderByAttemptsAsc()` and `findAllByOrderByCreatedAtDesc()` methods from `GameRepository`.
- Improved code clarity and reduced business logic complexity in the service layer.
